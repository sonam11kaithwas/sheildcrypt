...
public class SipServiceCommand implements SipServiceConstants {
    ...
    protected static final String ACTION_RESET_ACCOUNT = "resetAccount";
    protected static final String ACTION_SET_REREGISTER_ACCOUNT = "setreregisterAccount";
    /**
    ...
    public static String setReRegisterAccount(Context context, SipAccountData sipAccount) {
        if (sipAccount == null) {
            throw new IllegalArgumentException("sipAccount MUST not be null!");
        }
        Log.d("pushnotification", "setReregister Account SIPSERVICECOMMAND>>>>>>>...setReRegisterAccount");
        String accountID = sipAccount.getIdUri();
        checkAccount(accountID);
        Intent intent = new Intent(context, SipService.class);
        intent.setAction(ACTION_SET_REREGISTER_ACCOUNT);
        intent.putExtra(PARAM_ACCOUNT_DATA, sipAccount);
        context.startService(intent);
        return accountID;
    }
    ...
    public static void resetAccount(Context context) {
        ...
        Intent intent = new Intent(context, SipService.class);
        intent.setAction(ACTION_RESET_ACCOUNT);
        context.startService(intent);
    }


    public static void getSpeaker(Context context, String accountID) {
        checkAccount(accountID);
        Intent intent = new Intent(context, SipService.class);
        intent.setAction(ACTION_CALL_SPEAKER);
        intent.putExtra(PARAM_ACCOUNT_ID, accountID);
        context.startService(intent);
    }
}
