...
public class SipCall extends Call {
    ...
    public void onCallState(OnCallStateParam prm) {
        try {
            ...
            account.getService().getBroadcastEmitter().callState(account.getData().getIdUri(), callID, callState, callStatus,
                    connectTimestamp, localHold,
                    localMute, localVideoMute);
    ...
    public void reinviteCallOnNetworkChange() {
        // return immediately if we are not changing the current state
        CallOpParam param = new CallOpParam();
        try {
            Log.d("networkchangereceiver", "update call" + getId());

            CallSetting opt = param.getOpt();
            opt.setAudioCount(1);
            opt.setVideoCount(1);
            // change by Sachin
            opt.setFlag(pjsua_call_flag.PJSUA_CALL_REINIT_MEDIA | pjsua_call_flag.PJSUA_CALL_UPDATE_CONTACT);
            param.setOpt(opt);
            reinvite(param);

        } catch (Exception exc) {
            ///networkchangereceiver
            Log.d("networkchangereceiver", "Error while trying to  update call", exc);
            Logger.error(LOG_TAG, "Error while trying to  update call", exc);
        }
    }
    ...
    private void handleAudioMedia(Media media) {
        account.getService().stopRingtone();//added by arvind
        AudioMedia audioMedia = AudioMedia.typecastFromMedia(media);
    ...
    public void setIncomingVideoFeed(Surface surface) {
        if (mVideoWindow != null) {
            ...
            try {
                //mVideoWindow.setFullScreen(true);
               // mVideoWindow.setFullScreen2(1);
