...
public class BroadcastEventReceiver extends BroadcastReceiver implements SipServiceConstants{
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (BroadcastEventEmitter.getAction(BroadcastEventEmitter.BroadcastAction.REGISTRATION).equals(action)) {
            ...
            onCallState(intent.getStringExtra(PARAM_ACCOUNT_ID),
                    ...
                    intent.getLongExtra(PARAM_CONNECT_TIMESTAMP, -1),
                    intent.getBooleanExtra(PARAM_HOLD, false),
                    intent.getBooleanExtra(PARAM_MUTE, false),
                    intent.getBooleanExtra(PARAM_VIDEO_MUTE, false));
    ...
    public void onOutgoingCall(String accountID, int callID, String number, boolean isVideo, boolean isVideoConference, boolean isTransfer) {
        ...
        //        AppConstants.TYPE="PTT";
        /*if(AppConstants.TYPE.equals("PTT"))
        {

            Log.e("call ptt by service","callptt"+AppConstants.TYPE);
        }
        else if(AppConstants.TYPE.equals("CALL"))
        {
            Log.e("callvoice","callvoice"+AppConstants.TYPE);

            try {
                Intent intentcall;
                if (isVideo) {

                    intentcall = new Intent(receiverContext, VideoActivity.class);
                } else {
                    intentcall = new Intent(receiverContext, CallActivity.class);
                }
                intentcall.putExtra(PARAM_ACCOUNT_ID, accountID);
                intentcall.putExtra(PARAM_CALL_ID, callID);
                intentcall.putExtra(PARAM_NUMBER, number);
                intentcall.putExtra(PARAM_CALL_TYPE, "OUT");
                ...
                receiverContext.startActivity(intentcall);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }*/
        try {
           // Intent intentcall = new Intent(receiverContext, CallActivity.class);
            Intent intentcall;
            if (isVideo) {
                 intentcall = new Intent(receiverContext, VideoActivity.class);
            } else {
