...
public class BroadcastEventEmitter implements SipServiceConstants {
    ...
    public static String NAMESPACE = "net.gotev";//"com.voismart";
    ...
    public BroadcastEventEmitter(Context context) {
        mContext = context;
        mySharedPreferences = SharedPrefrence.getInstance(mContext);
    }
    ...
    public synchronized  void callState(String accountID, int callID, int callStateCode, int callStateStatus,
        ...
        if(callStateCode == 6)
        {
            try {System.out.println("call disconnected f");
        ...
        }else if (callStateCode == 4)
        {

        }

        System.out.println("call state==== brodcastemitter calling");
        final Intent intent = new Intent();
        ...
        intent.putExtra(PARAM_CALL_STATUS, callStateStatus);
        intent.putExtra(PARAM_CONNECT_TIMESTAMP, connectTimestamp);
        intent.putExtra(PARAM_HOLD, isLocalHold);
        intent.putExtra(PARAM_MUTE, isLocalMute);
        intent.putExtra(PARAM_VIDEO_MUTE, isLocalVideoMute);
