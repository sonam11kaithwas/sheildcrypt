package com.advantal.shieldcrypt.parser;
...
import com.advantal.shieldcrypt.crypto.axolotl.BrokenSessionException;
import com.advantal.shieldcrypt.crypto.axolotl.NotEncryptedForThisDeviceException;
import com.advantal.shieldcrypt.crypto.axolotl.OutdatedSenderException;
...
import com.advantal.shieldcrypt.entities.ReceiptRequest;
...
public class MessageParser extends AbstractParser implements OnMessagePacketReceived {
    ...
    private static String extractStanzaId(Element packet, Jid by) {
        for (Element child : packet.getChildren()) {
            if (child.getName().equals("stanza-id") && Namespace.STANZA_IDS.equals(child.getNamespace()) && by.equals(InvalidJid.getNullForInvalid(child.getAttributeAsJid("by")))) {
    ...
    public void onMessagePacketReceived(Account account, MessagePacket original) {
        ...
        /***********this statement for khud k node k liye hai************/


        if (original.getChildren() != null && original.getChildren().get(0) != null && original.getChildren().get(0).getAttributes() != null && original.getChildren().get(0).getAttributes().get("xmlns").equals("urn:xmpp:chat-markers:0")) {
        } else {
            if (original.getBody() == null) return;
        }
        if (original != null && original.getFrom() != null && original.getTo() != null && original.getFrom().asBareJid().equals(original.getTo().asBareJid()))
            return;


        if (original.getSubject() != null && original.getSubject().content != null && !original.getSubject().content.equals("")) {
            grpNm = "SENDERNAME" + original.getFrom().asBareJid().toString();
            original.setFrom(Jid.of(original.getSubject().content));
        }
        ...
        if (body != null) {
            Log.e("Message Recieve-------", body.content);
        }
...
//        boolean isTypeGroupChat  = isGrp;// = packet.getType() == MessagePacket.TYPE_GROUPCHAT;
...
//            final boolean conversationIsProbablyMuc = isGrp;//isTypeGroupChat || mucUserElement != null || account.getXmppConnection().getMucServersWithholdAccount().contains(counterpart.getDomain().toEscapedString());
           ...
           /* if (from.asBareJid().toString().contains("@broadcast")) {
                ResponseItem model = MyAppDataBase.Companion.getUserDataBaseAppinstance(MyApp.Companion.getAppInstance()).groupDao().getGroupDetailsById(from.asBareJid().toString().split("@")[0]);
                if (model != null) {
                    conversation = mXmppConnectionService.findOrCreateConversationForMessage(account, counterpart.asBareJid(), conversationIsProbablyMuc, false, query, false);
                }
            } else {
                ...
                if (replacedMessage != null) {
                    ...
                    if (fingerprintsMatch && (trueCountersMatch || !conversationMultiMode || mucUserMatches) && !duplicate) {
                        ...
                        synchronized (replacedMessage) {
                            ...
                            if (mXmppConnectionService.confirmMessages() && replacedMessage.getStatus() == Message.STATUS_RECEIVED && (replacedMessage.trusted() || replacedMessage.isPrivateMessage()) //TODO do we really want to send receipts for all PMs?
                                    && remoteMsgId != null && !selfAddressed && !isTypeGroupChat) {
    ...
    private void processMessageReceipts(final Account account, final MessagePacket packet, final String remoteMsgId, MessageArchiveService.Query query) {
        ...
        if (query == null) {
            ...
            if (receiptsNamespaces.size() > 0) {
                final MessagePacket receipt = mXmppConnectionService.getMessageGenerator().received(account, packet.getFrom(), remoteMsgId, receiptsNamespaces, packet.getType());
