...
import com.advantal.shieldcrypt.entities.Message.FileParams;
import com.advantal.shieldcrypt.entities.RtpSessionStatus;
...
import com.advantal.shieldcrypt.ui.service.AudioPlayer;
import com.advantal.shieldcrypt.ui.text.DividerSpan;
import com.advantal.shieldcrypt.ui.text.QuoteSpan;
import com.advantal.shieldcrypt.ui.util.AvatarWorkerTask;
import com.advantal.shieldcrypt.ui.util.MyLinkify;
import com.advantal.shieldcrypt.ui.util.QuoteHelper;
import com.advantal.shieldcrypt.ui.util.ViewUtil;
import com.advantal.shieldcrypt.ui.widget.ClickableMovementMethod;
import com.advantal.shieldcrypt.utils.CryptoHelper;
import com.advantal.shieldcrypt.utils.Emoticons;
...
public class MessageAdapter extends ArrayAdapter<Message> {
    ...
    private String tempGrpRecNm = "";
    private List<String> highlightedTerm = null;
    private OnContactPictureClicked mOnContactPictureClickedListener;
    private OnContactPictureLongClicked mOnContactPictureLongClickedListener;
    private boolean mUseGreenBackground = false;
    private int FLAGRECEIVEDSEND = 1;

    public MessageAdapter(final XmppActivity activity, final List<Message> messages, final boolean forceNames, ConversationFragment conversationFragment) {
    ...
    public MessageAdapter(final XmppActivity activity, final List<Message> messages, ConversationFragment conversationFragment) {
        this(activity, messages, false, conversationFragment);
    ...
    private void displayStatus(ViewHolder viewHolder, Message message, int type, boolean darkBackground) {
        ...
        switch (message.getMergedStatus()) {
            ...
            case Message.STATUS_SEND_RECEIVED:
                if (!message.getCounterpart().asBareJid().toString().contains("@broadcast")) {
//                viewHolder.indicatorReceived.setImageResource(darkBackground ? R.drawable.ic_done_white_18dp : R.drawable.ic_done_black_18dp);
                    viewHolder.indicatorReceived.setImageResource(R.drawable.ic_done_white_18dp);
//                    viewHolder.indicatorReceived.setAlpha(darkBackground ? 0.7f : 0.57f);
                    viewHolder.indicatorReceived.setVisibility(View.VISIBLE);
                }
                break;
            case Message.STATUS_SEND_DISPLAYED:
                Log.e("Background : ", "" + darkBackground);
                if (!message.getCounterpart().asBareJid().toString().contains("@broadcast")) {
//                    viewHolder.indicatorReceived.setAlpha(darkBackground ? 0.7f : 0.57f);
                    viewHolder.indicatorReceived.setVisibility(View.VISIBLE);
                    viewHolder.indicatorReceived.setImageResource(R.drawable.ic_double_tick);
                }
                break;
...
//                viewHolder.indicator.setAlpha(0.57f);
        ...
        if (message.getCounterpart() != null && message.getCounterpart().asBareJid() != null && message.getCounterpart().asBareJid().toString().contains("@broadcast") && message.getBodyLanguage() != null && !message.getBodyLanguage().equals("")) {
            String[] str = message.getBodyLanguage().split("SENDERNAME");
            if (str.length > 0) {
                String i = str[0].split("@")[0];
                if (viewHolder.message_sender_nm != null) {
//                    if (!tempGrpRecNm.equals(MyAppDataBase.Companion.getUserDataBaseAppinstance(MyApp.Companion.getAppInstance()).contactDao().getUser(i))) {
                        viewHolder.message_sender_nm.setVisibility(View.VISIBLE);
                        viewHolder.message_sender_nm.setText(MyAppDataBase.Companion.getUserDataBaseAppinstance(MyApp.Companion.getAppInstance()).contactDao().getUser(i));
//                        tempGrpRecNm = viewHolder.message_sender_nm.getText().toString();
//                    }
                }
                message.setBodyLanguage(str[str.length - 1]);
            }

        } else if (viewHolder.message_sender_nm != null) {
            viewHolder.message_sender_nm.setVisibility(View.GONE);
        }

        final String bodyLanguage = "";
        if (message.getBodyLanguage() != null && !message.getBodyLanguage().equals("") && !message.getBodyLanguage().contains("@")) {
            message.getBodyLanguage();
        }
        final String bodyLanguageInfo = (bodyLanguage == null || bodyLanguage.equals("")) ? "" : String.format(" \u00B7 %s", bodyLanguage.toUpperCase(Locale.US));
    ...
    private void displayInfoMessage(ViewHolder viewHolder, CharSequence text, boolean darkBackground) {
        ...
        if (viewHolder.message_sender_nm != null)
            viewHolder.messageBody.setLinkTextColor(Color.BLUE);
        else viewHolder.messageBody.setLinkTextColor(Color.GREEN);
        if (darkBackground) {
...
//     *//*
    ...
    private void displayTextMessage(final ViewHolder viewHolder, final Message message, boolean darkBackground, int type) {
        ...
        //        if (FLAGRECEIVEDSEND == 2)
//            viewHolder.messageBody.setTextColor(ContextCompat.getColor(activity, R.color.white));
//        else
//        viewHolder.messageBody.setTextColor(ContextCompat.getColor(activity, R.color.black_clr));
        if (darkBackground) {
...
//        viewHolder.messageBody.setTypeface(null, Typeface.NORMAL);
        ...
        if (message.getBody() != null) {
            ...
            SpannableStringBuilder body = message.getMergedBody();//new SpannableStringBuilder( message.getBody().toString());
            ...
            if (body.toString().contains("-")) {
                String[] strArray = body.toString().split("-");
                String str = strArray[0];
                viewHolder.messageBody.setText(str);

            } else viewHolder.messageBody.setText(body);
            if (viewHolder.message_sender_nm != null)
                viewHolder.messageBody.setLinkTextColor(Color.BLUE);
            else viewHolder.messageBody.setLinkTextColor(Color.GREEN);
    ...
    private void displayDownloadableMessage(ViewHolder viewHolder, final Message message, String text, final boolean darkBackground) {
        ...
        viewHolder.download_button.setOnClickListener(v -> conversationFragment.downloadFile(message));
//                ConversationFragment.downloadFile(activity, message)
    ...
    private void loadMoreMessages(Conversation conversation) {
        ...
        if (query != null) {
//            Toast.makeText(activ/ity, R.string.fetching_history_from_server, Toast.LENGTH_LONG).show();
        } else {
//            Toast.makeText(activity, R.string.not_fetching_history_retention_period, Toast.LENGTH_SHORT).show();
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        final Conversational conversation = message.getConversation();//((Conversation) conversation).getLastEditableMessage()
        ...
        if (view == null) {
            ...
            switch (type) {
                ...
                case RECEIVED:
                    ...
                    viewHolder.image = view.findViewById(R.id.message_image);
                    viewHolder.messageBody = view.findViewById(R.id.message_body);
                    viewHolder.message_sender_nm = view.findViewById(R.id.message_sender_nm);
//                    viewHolder.message_sender_nm.setVisibility(View.GONE);
//                    viewHolder.message_sender_nm.setText("");
                    viewHolder.time = view.findViewById(R.id.message_time);
                    viewHolder.indicatorReceived = view.findViewById(R.id.indicator_received);
...
//            viewHolder.indicatorReceived.setAlpha(isDarkTheme ? 0.7f : 0.57f);
            ...
            if ("LOAD_MORE".equals(message.getBody())) {
                ...
                boolean showAvatar;
                String str = MyAppDataBase.Companion.getUserDataBaseAppinstance(MyApp.Companion.getAppInstance()).contactDao().getUserProfilePick(message.getConversation().getContact().getDisplayName());
                if (conversation.getMode() == Conversation.MODE_SINGLE) {
                    showAvatar = true;
                    AvatarWorkerTask.loadAvatar(message, viewHolder.contact_picture, R.dimen.avatar_on_status_message, str);
...
//                    viewHolder.contact_picture.setVisibility(View.VISIBLE);
            ...
            AvatarWorkerTask.loadAvatar(message, viewHolder.contact_picture, R.dimen.avatar, str);
        }
...
//                    viewHolder.message_box.setBackgroundResource(bubble);
    ...
    private static class ViewHolder {
        ...
        protected TextView message_sender_nm;
