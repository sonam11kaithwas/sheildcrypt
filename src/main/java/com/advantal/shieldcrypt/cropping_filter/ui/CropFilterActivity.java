...
public class CropFilterActivity extends AppCompatActivity implements CropImageView.OnSetImageUriCompleteListener, CropImageView.OnCropImageCompleteListener {
    private final ArrayList<FilterData> filtersList = new ArrayList<>();
    ...
    public void setAdapter(Bitmap bitmap) {
        setFiltersData();
        filtersAdapter = new FiltersAdapter(this, filtersList, bitmap, new FiltersAdapter.Callback() {
            @Override
                ...
                if (rlCropMainView.getVisibility() != View.VISIBLE) {
                    ...
                    photoEditorView.saveFilter(new OnSaveBitmap() {
                        ...
                        public void onBitmapReady(@Nullable Bitmap bitmap) {
                            if (bitmap != null) mCropImageView.setImageBitmap(bitmap);
                        }
    ...
    public void loadData(Bitmap bitmap) {
        try {
            if (bitmap != null && !bitmap.equals("")) {
                ...
                String path = MediaStore.Images.Media.insertImage(CropFilterActivity.this.getContentResolver(), bitmap, "SK_" + System.currentTimeMillis(), null);
                if (path != null && !path.isEmpty() && Uri.parse(path) != null) {
                    if (getIntent().hasExtra("ACTION") && getIntent().getStringExtra("ACTION") != null && !getIntent().getStringExtra("ACTION").isEmpty())
                        ViewGroupDetailsActivity.MyObject.croppedBitmap = Uri.parse(path).toString();
                    else ProfileActivity.MyObject.croppedBitmap = Uri.parse(path).toString();
                }
            } else {
                Toast.makeText(CropFilterActivity.this, "Something went wrong", Toast.LENGTH_LONG).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
